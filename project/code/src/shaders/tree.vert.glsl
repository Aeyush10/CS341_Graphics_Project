...
void main () {
    ...
    vec4 vertex_position_temp = view * model * position_v4;
    vec3 vertex_position_eye = vertex_position_temp.xyz;
    surface_normal = normalize(mat_normals*normal);	
    view_vector = -normalize(vertex_position_eye);
    lighting_vector = normalize(light_position.xyz - vertex_position_eye);


    vec2 local_coord =  2. * scale * offset.xy * viewer_scale;
	vec2 v2f_tex_coords = viewer_position + local_coord;
    vec3 noise = tex_fbm_for_terrain(v2f_tex_coords);

	// noise value is in the range 0..1, so we need to transform it to -0.5..0.5
    float height = noise.x - 0.5;
